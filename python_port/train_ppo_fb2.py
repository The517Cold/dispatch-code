...
def main():
    ...
    out_path = os.path.join(base_dir, "results/Reference_ppo_outputs/reference_ppo_result_case2_test2.txt")
    progress_path = os.path.join(base_dir, "results/Reference_ppo_outputs/reference_ppo_progress_case2_test2.txt")
    
    try:
        default_train_files = ["1-3.txt","1-3-a.txt",
                               "1-4.txt","1-4-a.txt",
                               "3-2.txt","3-2-a.txt",
                               "3-4.txt","3-4-c.txt",]
        train_files = _env_list("GCN_PPO_HQ_TRAIN_FILES") or default_train_files
        # 训练文件搜索路径
        train_roots = ["resources/resources_new/train/case1/test2",
                        "resources/resources_new/train/case2/test2"]
        ...
        ckpt_path = checkpoint_path(base_dir, "Reference_checkpoint/reference_ppo_checkpoint_case2_test2", signature)

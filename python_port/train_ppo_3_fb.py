...
class PetriNetGCNPPOProHQ(PetriNetGCNPPOPro):
    def __init__(self, petri_net, end, pre, post, min_delay_p, env_pool=None, **kwargs):
        ...
        default_params = {
            ...
            "ppo_epochs": _env_int("GCN_PPO_HQ_PPO_EPOCHS", 4),
            "target_kl": _env_float("GCN_PPO_HQ_TARGET_KL", 0.05),
            "entropy_coef_start": _env_float("GCN_PPO_HQ_ENTROPY_START", 0.09),
...
def main():
    ...
    out_path = os.path.join(base_dir, "results/Reference_ppo_outputs/reference_ppo_result_case1_test1.txt")
    progress_path = os.path.join(base_dir, "results/Reference_ppo_outputs/reference_ppo_progress_case1_test1.txt")
    ...
    try:
        ...
        # 训练文件搜索路径
        train_roots = ["resources/resources_new/train/case1",
                        "resources/resources_new/train/case2"]
        ...
        if fast_mode:
            ...
            max_train_steps = min(600000, max(50000, base_steps + extra_steps))
        ...
        ckpt_path = checkpoint_path(base_dir, "Reference_checkpoint/reference_ppo_checkpoint_case1_test1", signature)

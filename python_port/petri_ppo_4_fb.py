...
class PetriNetGCNPPOPro(AbstractSearch):
    ...
    def _select_training_env(self, epoch_idx: int):
        if not hasattr(self, "env_pool") or len(self.env_pool) <= 1:
            return None
        # 按复杂度排序(从简单到复杂)
    ...
    def switch_environment(self, env_dict):
        ...
        self.current_env_name = env_dict.get("name", "default")
        self._mask_cache = {}
    ...
    def _update_ppo(self) -> Tuple[float, float, float]:
        # 没有经验不更新
        ...
        for i in reversed(range(len(self.buffer.rewards))):
            if self.buffer.is_terminals[i]: 
                gae = 0
            next_value = 0.0 if self.buffer.is_terminals[i] or i == len(self.buffer.rewards) - 1 else old_values[i + 1].item()
            delta = self.buffer.rewards[i] + self.gamma * next_value - old_values[i].item()
    ...
    def train_model(self):
        self._log(f"=== [PPO-PRO] Training Started | Target Steps: {self.max_train_steps} ===")
        saved_env_name = self.current_env_name
        total_steps = 0
        epoch_idx = 0
        ...
        while total_steps < self.max_train_steps:
            ...
            if hasattr(self, "env_pool") and len(self.env_pool) > 1:
            ...
            steps_collected, ep_rewards, ep_makespans = self._collect_rollouts(self.steps_per_epoch)  # 收集经验,走steps_per_epoch步
            total_steps += steps_collected
            
            a_loss, c_loss, kl = self._update_ppo()  # 更新模型
        ...
        if hasattr(self, "env_pool"):
            for env in self.env_pool:
                if env.get("name") == saved_env_name:
                    self.switch_environment(env)  # 调用这个函数是为了把环境还原至初始状态
                    break
        
        
        env_record = getattr(self, "best_records", {}).get(self.current_env_name, {})  # 打印的是最后一个epoch所进行训练的网文件的最佳makespan,打不打印都可以

...
from python_port.petri_gcn_ppo_4_1 import PetriNetGCNPPOPro
...
def _safe_get_extra_info(extra_info: Dict[str, Any], key: str, default: Any = None) -> Any:
...
def _compute_step_schedule(context: Dict[str, Any], expert_steps: int = 0) -> Dict[str, Any]:
    """
        根据网的复杂度(状态数、转换数、约束数)计算训练/推理的步数预算.
    """
    ...
    return {
        ...
        "step_reference_source": step_reference_source,  # 步数来源
...
def _build_search_instance_pro(
    context: Dict[str, Any],
    petri_net: Any,
    schedule: Dict[str, Any],
    device: torch.device,
    ...
    return PetriNetGCNPPOPro(
        ...
        max_train_steps=0,
        # steps_per_epoch=schedule["min_steps"],
        steps_per_epoch = 1024,
        minibatch_size=64,
        ppo_epochs=1,
        # beam_width=100,
        # beam_depth=schedule["inference_max_steps"],
        # beam_depth = 800,
        verbose=verbose,
        device=str(device),
    )
...
def _run_single_inference_hq(
    ...
    try:
        search = _build_search_instance_hq(context, petri_net, schedule, verbose=verbose)
        ...
        result.success = bool(_safe_get_extra_info(extra_info, "reachGoal", False))
        ...
        result.stop_reason = str(_safe_get_extra_info(extra_info, "inferenceStopReason", ""))
        result.deadlock_reason = str(_safe_get_extra_info(extra_info, "inferenceDeadlockReason", ""))
...
def _run_single_inference_pro(
    ...
    try:
        search = _build_search_instance_pro(context, petri_net, schedule, device, verbose=verbose)
        
        if actor_state:
            actor_state_device = {k: v.to(device) if isinstance(v, torch.Tensor) else v for k, v in actor_state.items()}
            load_compatible_state(search.model.actor_net, actor_state_device)
            search.model.actor_net.set_graph(context["pre"], context["post"])
        if critic_state:
            critic_state_device = {k: v.to(device) if isinstance(v, torch.Tensor) else v for k, v in critic_state.items()}
            load_compatible_state(search.model.value_head, critic_state_device)
...
def _compare_results(il_res: InferenceResult, rl_res: InferenceResult, ref_res: Optional[InferenceResult] = None) -> str:
    """
        ┌─────────────────────────────────────────────────────────────────────────────────┐
        │                          结果比较优先级                                            │
        └─────────────────────────────────────────────────────────────────────────────────┘
        ...
        优先级 3: 变迁数越少越好
        ────────────────────────────
        makespan 相同时，按 trans_count 升序排序

        优先级 4: 全部失败
        ────────────────────────────
        返回 "Tie (All Failed)"
    """
    results = [("IL", il_res), ("IL+RL", rl_res)]
    if ref_res is not None:
        results.append(("REF", ref_res))
    
    valid_results = [(name, res) for name, res in results if res.success]
    
    if not valid_results:
        return "Tie (All Failed)"
    
    valid_results.sort(key=lambda x: (x[1].makespan, x[1].trans_count))
    
    return valid_results[0][0]


def _get_optimal_result(
    il_res: InferenceResult, 
    rl_res: InferenceResult, 
    ref_res: Optional[InferenceResult],
    optimal_method: str
) -> InferenceResult:
    """
        根据最优方法返回最优结果
    ...
    if optimal_method == "IL":
        return il_res
    elif optimal_method == "IL+RL":
        return rl_res
    elif optimal_method == "REF" and ref_res is not None:
        return ref_res
    elif "Tie" in optimal_method and il_res.success:
        return il_res
    return il_res
...
class BatchInferenceEvaluator:
    def __init__(
        ...
        ref_checkpoint_path: Optional[str] = None,
        max_workers: int = 4,
        verbose: bool = False,
    ):
        ...
        self.ref_actor_state: Dict[str, Any] = {}
        self.ref_critic_state: Dict[str, Any] = {}
    ...
    def _load_checkpoints(self):
        ...
        if self.ref_checkpoint_path:
            self.ref_actor_state, self.ref_critic_state = self._load_checkpoint_states(
                self.ref_checkpoint_path, "REF"
            )
    ...
    def _process_single_file(self, file_path: str) -> FileInferenceOutput:
        ...
        try:
            ...
            il_result = _run_single_inference_hq(
                context=context,
            ...
            il_rl_result = _run_single_inference_hq(
                context=context,
                petri_net=petri_net.clone(),
                schedule=schedule,
            ...
            ref_result = None
            if self.ref_checkpoint_path and self.ref_actor_state:
                ref_result = _run_single_inference_pro(
                    context=context,
                    petri_net=petri_net.clone(),
                    schedule=schedule,
                    actor_state=self.ref_actor_state,
                    critic_state=self.ref_critic_state,
                    method_name="REF",
                    device=self.device,
                    verbose=self.verbose,
                )
            ...
            if ref_result:
                output.ref_result = asdict(ref_result)
            
            best_method = _compare_results(il_result, il_rl_result, ref_result)
            output.optimal_method = best_method
            
            optimal_res = _get_optimal_result(il_result, il_rl_result, ref_result, best_method)
            if optimal_res.success:
                output.optimal_metrics = {
                    "makespan": optimal_res.makespan,
                    "trans_count": optimal_res.trans_count,
                }
...
def main():
    ...
    ref_ckpt = os.environ.get("REF_CHECKPOINT_PATH", "")
    if not ref_ckpt:
        ref_ckpt = os.path.join(checkpoints_dir, "gcn_ppo_pro_hq_general_family2_test2_2-1-5_a8c5c5621aec1fd16a9b0d84979b533e831fa345.pt")
    
    # test_files = [
    #     os.path.join(base_dir, "test", "1-1-13.txt"),
    #     os.path.join(base_dir, "test", "1-2-13.txt"),
    #     os.path.join(base_dir, "test", "1-3-13.txt"),
    #     os.path.join(base_dir, "test", "1-4-13.txt"),
    #     os.path.join(base_dir, "test", "1-5-13.txt"),
    #     os.path.join(base_dir, "test", "1-1-6.txt"),
    #     os.path.join(base_dir, "test", "1-2-6.txt"),
    #     os.path.join(base_dir, "test", "1-3-6.txt"),
    #     os.path.join(base_dir, "test", "1-4-6.txt"),
    #     os.path.join(base_dir, "test", "1-5-6.txt"),
    # ]
    test_files = [
            os.path.join(resources_dir,  "2-1-4.txt"),
            os.path.join(resources_dir,  "2-1-9.txt"),
            os.path.join(resources_dir,  "2-1-11.txt"),
            os.path.join(resources_dir,  "2-1-14.txt"),
            os.path.join(resources_dir,  "2-2-4.txt"),
            os.path.join(resources_dir,  "2-2-9.txt"),
            os.path.join(resources_dir,  "2-2-11.txt"),
            os.path.join(resources_dir,  "2-2-14.txt"),
            os.path.join(resources_dir,  "2-3-4.txt"),
            os.path.join(resources_dir,  "2-3-9.txt"),
            os.path.join(resources_dir,  "2-3-11.txt"),
            os.path.join(resources_dir,  "2-3-14.txt"),
            os.path.join(resources_dir,  "2-4-4.txt"),
            os.path.join(resources_dir,  "2-4-9.txt"),
            os.path.join(resources_dir,  "2-4-11.txt"),
            os.path.join(resources_dir,  "2-4-14.txt"),
            os.path.join(resources_dir,  "2-5-4.txt"),
            os.path.join(resources_dir,  "2-5-9.txt"),
            os.path.join(resources_dir,  "2-5-11.txt"),
            os.path.join(resources_dir,  "2-5-14.txt"),
            ]
    ...
    output_path = os.path.join(results_dir, "batch_inference_result2.json")

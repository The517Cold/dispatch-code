...
class PetriNetGCNPPOPro(AbstractSearch):
    def __init__(
        ...
        mixed_rollout: bool = False,  # 是否启用混合经验收集模式
    ...
    def _collect_mixed_rollouts(self, total_steps: int):
        """
        混合经验收集模式：从多个环境中各收集一部分经验，每个环境收集后立即更新 PPO

        与串行模式的区别：
        - 串行模式：每个 epoch 只在一个环境上收集经验并更新
        - 混合模式：每个 epoch 在多个环境上轮流收集并更新，保证梯度信号来自多个环境

        注意：由于不同 Petri 网的特征维度可能不同，经验不能混合到同一个 buffer 中。
        因此采用"收集-更新-清空"的循环方式，每个环境独立收集和更新。
